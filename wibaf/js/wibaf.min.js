	...
	var adaptationEngine = {
		...
		delete_node : function(selector, arg) {
		    ...
		    $.makeArray($(sel)).forEach(function(link) {
		        ...
		        link = $(link);
		        linkText = link.text().trim().toLowerCase().replace(/\s+/g, "_").replace(/[^a-z0-9_]/gmi, "") + suffix + "_accessed";
	...
	function init() {
		...
		return {
			...
			addVisit : function(callback) {
			    var varName = (document.title + "_accessed").replace(/\s+/g, "_").replace(/[^a-z0-9_]/gmi, "").trim().toLowerCase();
...
var userModel = (function UMEngine() {
    ...
    function init() {
        ...
        function init(name, value, type, url, feedback, domain, callback) {
            name = name.replace(/\s+/g, "_").replace(/[^a-z0-9_]/gmi, "").trim().toLowerCase();
        ...
        function setAllServer(value, callback) {
            getAll(function(values) {
                for (var i = 0; i < values.length; i++) {
                    setServer(values[i].name, {
                        send: value
                    });
                }
                if(callback) {
...
var ServerAPI = function() {
	...
	function init(endpoint, callback) {
	        ...
	        if(endpoint) {
	            ...
	            console.warn("WARNING: No endpoint has been defined. Please define an endpoint or write your own code to send data to the server");
	        }
	    }
	...
	function init(dictionary) {
	    ...
	    return {
	        ...
	        getDictionary : function() {
	            return {
	                get : function(key) {
                        if(dictionary) return dictionary[key];
                        else return null;
	                }
	            }
	        }
